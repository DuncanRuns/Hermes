...
public class WorldStateUpdater implements BiConsumer<JsonObject, MinecraftServer> {
    ...
    public void accept(JsonObject json, MinecraftServer server) {
        json.add("world", Optional.ofNullable(server).filter(s -> HermesMod.getSavePath(s) != null).map(s -> HermesCore.pathToJsonObject(HermesMod.getSavePath(s).normalize().toAbsolutePath())).orElse(null));
    }
}
